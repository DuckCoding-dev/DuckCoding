 ...
 */
interface BalanceConfigBackend {
  id: string;
  name: string;
  endpoint: string;
  method: 'GET' | 'POST';
 ...
 * 转换后端格式到前端格式
 */
function toFrontendConfig(backend: BalanceConfigBackend) {
  return {
    id: backend.id,
    name: backend.name,
 ...
 * 转换前端格式到后端格式
 */
function toBackendConfig(frontend: any): BalanceConfigBackend {
  return {
    id: frontend.id,
    name: frontend.name,
 ...
 * 保存新的余额监控配置
 */
export async function saveBalanceConfig(config: any): Promise<void> {
 ...
 * 更新现有的余额监控配置
 */
export async function updateBalanceConfig(config: any): Promise<void> {
 ...
 * 这个命令由前端在首次加载时自动调用
 */
export async function migrateBalanceFromLocalstorage(configs: any[]): Promise<number> {
